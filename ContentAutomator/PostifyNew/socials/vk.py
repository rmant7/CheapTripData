...
from utils.methods import get_post_data_by_path, check_if_post_exists, record_post_info, update_post_info
...
async def add_post_with_pic(token, owner_id, post_path, post_date=None):
    post_text, images = await get_post_data_by_path(post_path, social='Vkontakte')
    image_path_list = []
    for image in images:
    ...
    task = account.post_on_wall(post_text=post_text, owner_id=owner_id,
                                user_id=user_id, publish_date=post_date,
                                images=photo_id_list)
    if task:
