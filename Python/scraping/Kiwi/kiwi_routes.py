...
def get_countries_by_continent(continent_codes: list[str]=['EU']):
...
def get_routes(from_: str, to_: str, vehicle_type: str) -> None:
    try:
        logger.info(f'Get routes for: {from_} - {to_}')
...
def process_response(from_: str, to_: str, routes: bytes, vehicle_type: str):
    try:
        ...
        if routes['_results'] != 0:
            save_json(routes, from_, to_, vehicle_type) # ['data'][0] - the cheapest route
    except Exception as err:
...
def main(vehicle_type: str='aircraft'):
    ...
    all_countries = get_countries_by_continent() + COUNTED_COUNTRIES
    all_routes = tuple(product(all_countries, repeat=2))
    for i in range(0, len(all_routes), BATCH_SIZE):
        routes = all_routes[i : i + BATCH_SIZE]
        try:
            with ThreadPoolExecutor() as executor:
                _ = [executor.submit(get_routes, *route, vehicle_type) for route in routes]
